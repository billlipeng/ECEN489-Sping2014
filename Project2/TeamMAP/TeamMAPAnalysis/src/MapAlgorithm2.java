...
public class MapAlgorithm2 implements InterpolationAlgorithm {
    @Override
    public ArrayList<DataPoint> analyze(ArrayList<DataPoint> dataSet) {
        ...
        for (DataPoint dp : dataSet) {
            bearing.add(dp.getBearing());
            speed.add(dp.getSpeed());
            latitude.add(dp.getLatitude());
            longitude.add(dp.getLongitude());
            time.add(dp.getTime());
        }
        ...
        for (int i = 0; i < bearing.size(); i++) {
            results.add(new DataPoint(time.get(i), longitude.get(i), latitude.get(i), bearing.get(i), speed.get(i)));
        }
