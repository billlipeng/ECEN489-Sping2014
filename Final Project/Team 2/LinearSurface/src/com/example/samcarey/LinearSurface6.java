...
public class LinearSurface6 {
	...
	public static void main(String []args){
		...
		DBHandler handler = new DBHandler("Antenna2.db");
		...
		for (int i=0; i<length; i++)
		{
			...
			//System.out.println(raw_array[i].toString());
		}
		origin[0] = raw_array[0].y();
		origin[1] = raw_array[0].x();
		toCsv(interpolate(toGrid(raw_array)), "after");
	}
	...
	public static Point_dt[] toGrid(Point_dt[] coords){
		...
		for (int i = 0 ; i < points.length ; i++){
			points[i] = new Point_dt(lonToX(coords[i].x()-origin[1]), latToY(coords[i].y()-origin[0]), coords[i].z());
	...
	public static Point_dt[][] interpolate(Point_dt[] data){		//linear interpolation
		...
		double xStart = function.bb_min().x();
		double xEnd = function.bb_max().x();
		double yStart = function.bb_min().y();
		double yEnd = function.bb_max().y();

		origin[0] = xToLon(xStart);
		origin[1] = yToLat(yStart);
		
		double deltaX = (xEnd-xStart)/(n-1);
		double deltaY = (yEnd-yStart)/(m-1);
		
		for (int i = m-1 ; i >= 0 ; i--){
			for (int j = 0 ; j < n ; j++){
				if (function.contains((xStart+j*deltaX),(yStart+i*deltaY))){
					interpolation[i][j] = new Point_dt((xStart+j*deltaX), (yStart+i*deltaY), function.z((xStart+j*deltaX), (yStart+i*deltaY)));
				}else{
					interpolation[i][j] = new Point_dt((xStart+j*deltaX), (yStart+i*deltaY), function.bb_min().z());
				}
			}
		}
