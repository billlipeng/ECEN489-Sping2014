...
public class FusionTablesSample extends Thread{
	static ArrayList<dataRow> row;
	public FusionTablesSample(ArrayList<dataRow> row_thread){
		FusionTablesSample.row = row_thread;
	}
  /**
   * Be sure to specify the name of your application. If the application name is {@code null} or
   * blank, the application will log a warning. Suggested format is "MyCompany-ProductName/1.0".
   */
  private static final String APPLICATION_NAME = "ECEN489-Project1/1.0";
  ...
  public void run(){
    try {
      ...
      String tableId = "1FMesioUT-ij0rsniXspTmCNYGvv_8QSdwG3oxmI";     //insert to existing table
...
*/
  ...
  private static void insertData(String tableId) throws IOException {
	  /*
	  String bulkinsert = "";
	  bulkinsert = "INSERT INTO " + tableId + " (RunID,SensorID,SensorType,SensorValue,Date,Time,Location) ";
	  bulkinsert += "VALUES(" + row.get(0).runID + ",'" + row.get(0).sensorID+ "','" + row.get(0).sensorType
			  + "'," + row.get(0).sensorValue + ",'" + row.get(0).date + "','" + row.get(0).time + "','" 
			  + row.get(0).latitude + ", "+row.get(0).longitude+ "')" ;
	  System.out.println(bulkinsert);
	  for(int i = 1; i < row.size(); i++)
	  {
		  bulkinsert += (", (" + row.get(i).runID + ",'" + row.get(i).sensorID+ "','" + row.get(i).sensorType
				  + "'," + row.get(i).sensorValue + ",'" + row.get(i).date + "','" + row.get(i).time + "','" 
				  + row.get(i).latitude + ", "+row.get(i).longitude+ "')" );
	  }
	  System.out.println(bulkinsert);
	  ...
	  */
	  ...
	  for(int i = 0; i < row.size(); i++)
	  {
		  ...
		  bulkinsert += "VALUES(" 
				  + row.get(i).runID + ",'" 
				  + row.get(i).sensorID + "','" 
				  + row.get(i).sensorType + "'," 
				  + row.get(i).sensorValue + ",'" 
				  + row.get(i).date + "','" 
				  + row.get(i).time + "','" 
				  + row.get(i).latitude + ", "
				  + row.get(i).longitude+ "')" ;
		  Sql sql = fusiontables.query().sql(bulkinsert);
		  try {
			  sql.execute();
			  Thread.sleep(200);
		  } catch (IllegalArgumentException e) {
